...
| `v1.3.0` | `v1.3.0-lt.1` | 2026-09-26 | **#203 — account links land in the portal, passwordless sign-in, staff "Send sign-in link"** (Story 42.9; Ambyflow `d1c9781` + `e43b3ee` → LT `7da26901`). Adapted to single-tenant `db`, fixed Legal Terminus copy, no audit entry. Email-link sign-in and `localhost` enabled on `legal-terminus-web`. **Not applicable:** API clients take a licence seat (`adc1e42` — LT has no plans or seats). **Not ported yet:** definition cache, non-blocking sends, compression, declared indexes (`1bbceda`). **LT-only at this tag:** mandatory lead fields and Indian mobile validation on the public site forms (`be1614d6`). |
