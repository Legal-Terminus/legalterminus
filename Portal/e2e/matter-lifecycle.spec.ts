 ...
 *    Storage files; verified indirectly via the documents list being gone).
 *  • E11-S09 archive — staff (incl. team member) can archive; only admin deletes.
 */

test('E05-S05: deleting a matter purges its documents', async ({ adminPage, clientPage }) => {
  ...
  try {
    ...
    // Matter is gone and its documents no longer resolve (subcollection purged).
