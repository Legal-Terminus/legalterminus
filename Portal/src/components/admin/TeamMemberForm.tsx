...
import { ArrowLeft, User, Mail, Phone, Briefcase, Shield, Calendar, AlertCircle, Save, X } from 'lucide-react';
...
export default function TeamMemberForm({ member, onClose, onSuccess, isFullPage = false }: TeamMemberFormProps) {
  ...
  if (isFullPage) {
    return (
      <div className="max-w-2xl mx-auto p-6">
        <button onClick={onClose} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors mb-6">
          <ArrowLeft className="w-4 h-4" />
          Back to Team Members
        </button>
        <div className="mb-6">
          <h1 className="text-xl font-bold text-gray-900">{member ? 'Edit Team Member' : 'Add Team Member'}</h1>
          <p className="text-sm text-gray-500 mt-1">{member ? "Update the member's information and role." : 'Create a new team member account and assign their role.'}</p>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-gray-900">{member ? 'Edit Team Member' : 'New Team Member'}</h2>
