...
const tabs = [
  { label: "Why Choose Annual Filing", id: "company" },
...
const AfcTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("afc-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("afc-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("afc-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`afc-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="afc-tabs-container">
          <div className="afc-tabs-card">
            <div
              ref={tabListRef}
              className="afc-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`afc-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="afc-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="afc-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="afc-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`afc-slim-item${index === activeIndex ? " active" : ""}`}
