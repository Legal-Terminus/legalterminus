...
const tabs = [
  { label: "Why Choose ESIC", id: "overview" },
...
const ESICRegTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("esic-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("esic-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("esic-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`esic-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="esic-tabs-container">
          <div className="esic-tabs-card">
            <div
              ref={tabListRef}
              className="esic-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`esic-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="esic-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="esic-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="esic-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`esic-slim-item${index === activeIndex ? " active" : ""}`}
