...
const tabs = [
  { label: "Why Choose PT", id: "company" },
...
const ProfTaxTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("proftax-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("proftax-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("proftax-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`proftax-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="proftax-tabs-container">
          <div className="proftax-tabs-card">
            <div
              ref={tabListRef}
              className="proftax-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`proftax-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="proftax-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="proftax-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="proftax-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`proftax-slim-item${index === activeIndex ? " active" : ""}`}
