...
const PvtTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("pro-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("pro-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("pro-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`pro-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="pro-tabs-container">
          <div className="pro-tabs-card">
            <div
              ref={tabListRef}
              className="pro-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`pro-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="pro-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="pro-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="pro-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`pro-slim-item${index === activeIndex ? " active" : ""}`}
