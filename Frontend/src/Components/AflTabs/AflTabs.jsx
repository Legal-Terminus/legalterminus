...
const AflTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("afl-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("afl-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("afl-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`afl-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="afl-tabs-container">
          <div className="afl-tabs-card">
            <div
              ref={tabListRef}
              className="afl-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`afl-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="afl-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="afl-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="afl-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`afl-slim-item${index === activeIndex ? " active" : ""}`}
