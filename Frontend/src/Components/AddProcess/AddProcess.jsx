...
const GSTProcess = () => {
  return (
    <section className="Add-Process-gst-wrapper">
      <h2 className="Add-Process-heading">
        Steps For Private Limited Company Registration in India
      </h2>
      <p className="Add-Process-subheading">
        The broad process of registering a Private Limited company involves
        the following steps:
      </p>

      <div className="Add-Process-timeline">
        <div className="Add-Process-timeline-line" />

        {steps.map((step, index) => (
          <div
            key={index}
            className={`Add-Process-timeline-item ${
              index % 2 === 0 ? "left" : "right"
            }`}
          >
            <div className="Add-Process-timeline-dot">{index + 1}</div>

            <div className="Add-Process-timeline-card">
              <h4>{step.title}</h4>
              <p>{step.text}</p>
            </div>
          </div>
