...
const LtopvtTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("ltopvt-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("ltopvt-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("ltopvt-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`ltopvt-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="ltopvt-tabs-container">
          <div className="ltopvt-tabs-card">
            <div
              ref={tabListRef}
              className="ltopvt-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`ltopvt-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="ltopvt-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="ltopvt-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="ltopvt-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`ltopvt-slim-item${index === activeIndex ? " active" : ""}`}
