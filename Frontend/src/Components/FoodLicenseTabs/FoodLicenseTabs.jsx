...
const tabs = [
  { label: "Why Choose FSSAI", id: "company" },
...
const FoodLicenseTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("food-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("food-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("food-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`food-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="food-tabs-container">
          <div className="food-tabs-card">
            <div
              ref={tabListRef}
              className="food-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`food-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="food-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="food-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="food-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`food-slim-item${index === activeIndex ? " active" : ""}`}
