...
const tabs = [
  { label: "Why Choose Udyam", id: "about" },
  { label: "Types", id: "who" },
...
const UdyamRegTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("udyam-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("udyam-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("udyam-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`udyam-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="udyam-tabs-container">
          <div className="udyam-tabs-card">
            <div
              ref={tabListRef}
              className="udyam-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`udyam-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="udyam-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="udyam-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="udyam-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`udyam-slim-item${index === activeIndex ? " active" : ""}`}
