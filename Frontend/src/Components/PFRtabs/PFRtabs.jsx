...
const tabs = [
  { label: "Why Choose Partnership", id: "company" },
...
const PFRTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("pfr-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("pfr-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("pfr-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`pfr-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="pfr-tabs-container">
          <div className="pfr-tabs-card">
            <div
              ref={tabListRef}
              className="pfr-tabs-list"
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="pfr-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="pfr-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="pfr-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`pfr-slim-item${index === activeIndex ? " active" : ""}`}
