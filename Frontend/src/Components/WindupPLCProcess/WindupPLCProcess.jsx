...
const WindupPLCProcess = () => {
  return (
    <section className="Windup-PLC-Pro-wrapper">
      <h2 className="Windup-PLC-Pro-heading">
        STEPS FOR PRIVATE LIMITED COMPANY REGISTRATION IN INDIA
      </h2>
      <p className="pv-Windup-PLC-Pro-subheading">
        The broad process of registering a Private Limited company involves
        the following steps:
      </p>

      <div className="Windup-PLC-Pro-timeline">
        <div className="Windup-PLC-Pro-timeline-line" />

        {steps.map((step, index) => (
          <div
            key={index}
            className={`Windup-PLC-Pro-timeline-item ${
              index % 2 === 0 ? "left" : "right"
            }`}
          >
            <div className="Windup-PLC-Pro-timeline-dot">{index + 1}</div>

            <div className="Windup-PLC-Pro-timeline-card">
              <h4>{step.title}</h4>
              <p>{step.text}</p>
            </div>
          </div>
...
export default WindupPLCProcess;
