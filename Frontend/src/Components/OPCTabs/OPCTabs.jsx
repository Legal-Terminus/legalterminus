...
const OPCTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("opc-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("opc-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("opc-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`opc-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="opc-tabs-container">
          <div className="opc-tabs-card">
            <div
              ref={tabListRef}
              className="opc-tabs-list"
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="opc-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="opc-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="opc-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`opc-slim-item${index === activeIndex ? " active" : ""}`}
