...
const PvtltdTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("pvtltd-nav-sections");
    if (!wrapper) return;
  ...
  return (
    <>
      ...
      <section className={`pvtltd-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="pvtltd-tabs-container">
          <div className="pvtltd-tabs-card">
            <div
              ref={tabListRef}
              className="pvtltd-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`pvtltd-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="pvtltd-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="pvtltd-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="pvtltd-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`pvtltd-slim-item${index === activeIndex ? " active" : ""}`}
