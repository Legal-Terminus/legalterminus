...
const tabs = [
  { label: "Why Close the Company", id: "company" },
...
const DpvtTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("dpvt-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("dpvt-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("dpvt-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`dpvt-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="dpvt-tabs-container">
          <div className="dpvt-tabs-card">
            <div
              ref={tabListRef}
              className="dpvt-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`dpvt-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="dpvt-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="dpvt-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="dpvt-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`dpvt-slim-item${index === activeIndex ? " active" : ""}`}
