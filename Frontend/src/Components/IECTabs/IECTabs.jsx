...
const IECTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("iec-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("iec-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("iec-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`iec-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="iec-tabs-container">
          <div className="iec-tabs-card">
            <div
              ref={tabListRef}
              className="iec-tabs-list"
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="iec-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="iec-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="iec-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`iec-slim-item${index === activeIndex ? " active" : ""}`}
