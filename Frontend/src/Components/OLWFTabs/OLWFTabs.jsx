...
const tabs = [
  { label: "Why Choose OLWF", id: "company" },
...
const OLWFTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("olwf-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("olwf-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("olwf-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`olwf-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="olwf-tabs-container">
          <div className="olwf-tabs-card">
            <div
              ref={tabListRef}
              className="olwf-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`olwf-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="olwf-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="olwf-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="olwf-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`olwf-slim-item${index === activeIndex ? " active" : ""}`}
