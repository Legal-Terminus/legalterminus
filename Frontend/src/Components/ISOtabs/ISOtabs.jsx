...
const PvtTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("iso-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("iso-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("iso-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`iso-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="iso-tabs-container">
          <div className="iso-tabs-card">
            <div
              ref={tabListRef}
              className="iso-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`iso-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="iso-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="iso-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="iso-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`iso-slim-item${index === activeIndex ? " active" : ""}`}
