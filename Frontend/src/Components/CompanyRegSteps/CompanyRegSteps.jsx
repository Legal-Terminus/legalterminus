...
const CompanyRegSteps = () => {
  return (
    <section className="crsteps-section">
      <div className="crsteps-container">
        <h2 className="crsteps-heading">
          Steps for Private Limited Company Registration in India
        </h2>
        <p className="crsteps-subheading">
          Nine steps. 7–10 working days end-to-end (assuming clean documents and a name that clears CRC on first try).
        </p>

        <div className="crsteps-grid">
          {steps.map((step, index) => (
            <article className="crsteps-card" key={index}>
              <div className="crsteps-card-top">
                <span className="crsteps-num">{String(index + 1).padStart(2, "0")}</span>
                <span className="crsteps-day">{step.day}</span>
              </div>
              <h3 className="crsteps-title">{step.title}</h3>
              <p className="crsteps-text">{step.text}</p>
            </article>
          ))}
        </div>
      </div>
...
export default CompanyRegSteps;
