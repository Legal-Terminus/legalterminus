...
const tabs = [
  { label: "Why Choose Incorporation", id: "company" },
...
const IncorporationTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("incorp-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("incorp-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("incorp-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`incorp-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="incorp-tabs-container">
          <div className="incorp-tabs-card">
            <div
              ref={tabListRef}
              className="incorp-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`incorp-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="incorp-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="incorp-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="incorp-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`incorp-slim-item${index === activeIndex ? " active" : ""}`}
