...
const LabourLicenseTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("labour-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("labour-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("labour-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`labour-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="labour-tabs-container">
          <div className="labour-tabs-card">
            <div
              ref={tabListRef}
              className="labour-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`labour-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="labour-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="labour-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="labour-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`labour-slim-item${index === activeIndex ? " active" : ""}`}
...
export default LabourLicenseTabs;
