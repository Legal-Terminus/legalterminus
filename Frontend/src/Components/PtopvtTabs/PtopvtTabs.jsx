...
const PtopvtTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("ptopvt-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("ptopvt-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("ptopvt-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`ptopvt-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="ptopvt-tabs-container">
          <div className="ptopvt-tabs-card">
            <div
              ref={tabListRef}
              className="ptopvt-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`ptopvt-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="ptopvt-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="ptopvt-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="ptopvt-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`ptopvt-slim-item${index === activeIndex ? " active" : ""}`}
