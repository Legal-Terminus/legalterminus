...
const tabs = [
  { label: "Why Close the LLP", id: "company" },
...
const DllpTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("dllp-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("dllp-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("dllp-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`dllp-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="dllp-tabs-container">
          <div className="dllp-tabs-card">
            <div
              ref={tabListRef}
              className="dllp-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`dllp-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="dllp-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="dllp-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="dllp-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`dllp-slim-item${index === activeIndex ? " active" : ""}`}
