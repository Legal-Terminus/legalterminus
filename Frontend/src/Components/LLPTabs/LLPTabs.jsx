...
const LLPTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("llp-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("llp-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("llp-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`llp-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="llp-tabs-container">
          <div className="llp-tabs-card">
            <div
              ref={tabListRef}
              className="llp-tabs-list"
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="llp-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="llp-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="llp-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`llp-slim-item${index === activeIndex ? " active" : ""}`}
