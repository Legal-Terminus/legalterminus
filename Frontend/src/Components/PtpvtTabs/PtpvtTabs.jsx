...
const PtpvtTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("ptpvt-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("ptpvt-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("ptpvt-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`ptpvt-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="ptpvt-tabs-container">
          <div className="ptpvt-tabs-card">
            <div
              ref={tabListRef}
              className="ptpvt-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`ptpvt-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="ptpvt-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="ptpvt-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="ptpvt-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`ptpvt-slim-item${index === activeIndex ? " active" : ""}`}
