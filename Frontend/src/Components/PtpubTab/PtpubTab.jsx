...
const tabs = [
  { label: "Why Convert to Public Ltd", id: "company" },
...
const PtpubTabs = () => {
  ...
  useEffect(() => {
    const wrapper = document.getElementById("ptpub-nav-sections");
    if (!wrapper) return;
    wrapper.classList.toggle("ptpub-sections-shifted", sidebarOpen);
    return () => wrapper.classList.remove("ptpub-sections-shifted");
  }, [sidebarOpen]);
  ...
  return (
    <>
      ...
      <section className={`ptpub-tabs-section${sidebarOpen ? " sidebar-open-mode" : ""}`}>
        <div className="ptpub-tabs-container">
          <div className="ptpub-tabs-card">
            <div
              ref={tabListRef}
              className="ptpub-tabs-list"
            ...
            >
              {tabs.map((tab, index) => (
                <button
                  ...
                  type="button"
                  className={`ptpub-tab${index === activeIndex ? " active" : ""}`}
      ...
      {/* ── Slim sidebar card ── */}
      {sidebarOpen && (
        <aside className="ptpub-slim-sidebar" role="navigation" aria-label="Page sections">
          <button
            className="ptpub-slim-close"
            onClick={() => setSidebarOpen(false)}
            aria-label="Close sidebar"
          >
            ✕
          </button>
          <nav className="ptpub-slim-nav">
            {tabs.map((tab, index) => (
              <button
                key={tab.id}
                type="button"
                className={`ptpub-slim-item${index === activeIndex ? " active" : ""}`}
